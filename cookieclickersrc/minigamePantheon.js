...
M.launch=function()
{
	...
	M.init=function(div)
	{
		...
		{
            // These comments are not from the source code, these are mine to understand the function better
			if (slot==god.slot) return false; // If there is no actual change return
			if (slot!=-1 && M.slot[slot]!=-1) // If gods in worship slots are being swapped
			{
				M.godsById[M.slot[slot]].slot=god.slot; // Gods occupying our desired slot is moved to our current one
				M.slot[god.slot]=M.slot[slot]; // Both in gods data and in M.slot
			}
			else if (god.slot!=-1) M.slot[god.slot]=-1; // If we moved from one slot worship to another empty one
			                                            // We put our previous slot as empty
			if (slot!=-1) M.slot[slot]=god.id; // If we are moving to a worship slot register that in M.slot
			god.slot=slot; // Change the slot in our gods data
			Game.recalculateGains=true; // Recalculate gains
		}
